...
int unzip(char * input_name, char * output_name)
{
    ...
    for (int i = 0; i < threads; i++) {
        ...
        pthread_create(&tid[i], NULL, parallelUnzip, args);
    }
    ...
    char c;
    FILE * zip_file = NULL;
    if((zip_file = fopen("output.txt", "a")) == NULL) {
